...
namespace CollegeCore.Utilities
{
    class CommonConstants
    {
        ...
        public static string QUERY_GET_TIMETABLE_BY_STUDENT_GROUP = "SELECT rr.[day], s.[group_id], s.[sub_group_id], s.[start_time], s.[end_time], rr.[room], t.[tag_name], su.[sub_name], su.[subject_Code], s.[consecutive_id], s.[parallel_id], s.[not_parallel_id], s.[session_status], l.[lecturer_name]  FROM (((([Sessions] s INNER JOIN [ReservedRoom] rr ON s.[session_id] = rr.[session_id]) INNER JOIN [tags] t ON t.[tag_code] = s.[tag_code]) INNER JOIN [subjects] su ON su.subject_id = s.subject_id) INNER JOIN [lecturers] l ON l.[idlecturer] = s.[lecturer_id]) WHERE s.[sub_group_id] = @Common;";
        public static string QUERY_GET_TIMETABLE_BY_ROOM = "SELECT rr.[day], s.[group_id], s.[sub_group_id], s.[start_time], s.[end_time], rr.[room], t.[tag_name], su.[sub_name], su.[subject_Code], s.[consecutive_id], s.[parallel_id], s.[not_parallel_id], s.[session_status], l.[lecturer_name]  FROM (((([Sessions] s INNER JOIN [ReservedRoom] rr ON s.[session_id] = rr.[session_id]) INNER JOIN [tags] t ON t.[tag_code] = s.[tag_code]) INNER JOIN [subjects] su ON su.subject_id = s.subject_id) INNER JOIN [lecturers] l ON l.[idlecturer] = s.[lecturer_id]) WHERE rs.[room] = @Common";
        public static string QUERY_GET_TIMETABLE_BY_LECTURER = "SELECT rr.[day], s.[group_id], s.[sub_group_id], s.[start_time], s.[end_time], rr.[room], t.[tag_name], su.[sub_name], su.[subject_Code], s.[consecutive_id], s.[parallel_id], s.[not_parallel_id], s.[session_status], l.[lecturer_name]  FROM (((([Sessions] s INNER JOIN [ReservedRoom] rr ON s.[session_id] = rr.[session_id]) INNER JOIN [tags] t ON t.[tag_code] = s.[tag_code]) INNER JOIN [subjects] su ON su.subject_id = s.subject_id) INNER JOIN [lecturers] l ON l.[idlecturer] = s.[lecturer_id]) WHERE l.[lecturer_name] = @Common;";
